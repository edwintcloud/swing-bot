...
the 09:30 ET open. A positive setup arms after $a_t$ reaches 0.1 basis points per second
squared on two consecutive bars while velocity is at least 1 basis point per second; a
negative setup mirrors the sign. After directional acceleration falls 0.075 basis points
from its armed peak, the strategy trades the reversal: a positive signal enters short and
a negative signal enters long. Setups expire after 10 seconds.

Each fill receives a reduce-only GTC trailing-stop-market order with a 40 basis point
...
position closes before rearming. A symbol waits five minutes between entries and is locked
out after two consecutive session losses. Missing five-second bars reset signal warmup so overnight or
