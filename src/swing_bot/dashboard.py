...
<aside><section class="section"><h2>Open positions</h2><div class="table-wrap"><table><thead><tr><th>Instrument</th><th>Side</th><th>Qty</th><th>Entry</th><th>Mark</th><th>Unrealized</th><th></th></tr></thead><tbody id="positions"></tbody></table></div><div class="notice">Flatten uses reduce-only GTC limit orders with a 1% execution collar and pauses new entries.</div></section><section class="section"><h2>Last control action</h2><div id="lastCommand" class="command"><p>No command processed.</p></div></section></aside></div></main>
<script>
...
document.getElementById('positions').innerHTML=(s.positions||[]).map(p=>`<tr><td>${esc(p.instrument_id)}</td><td class="side ${esc(p.side)}">${esc(p.side)}</td><td>${esc(p.quantity)}</td><td>${num.format(p.avg_px_open)}</td><td>${num.format(p.mark_price)}</td><td class="${cls(p.unrealized_pnl)}">${money.format(p.unrealized_pnl)}</td><td><button class="danger position-flatten" data-instrument="${esc(p.instrument_id)}" title="Flatten position" aria-label="Flatten ${esc(p.instrument_id)}">&times;</button></td></tr>`).join('')||'<tr><td colspan="7">No open positions</td></tr>';
...
document.getElementById('positions').onclick=e=>{const b=e.target.closest('button[data-instrument]');if(b&&confirm(`Pause entries and flatten ${b.dataset.instrument}?`))command('/api/flatten',{instrument_id:b.dataset.instrument})};
</script></body></html>"""
